...
def plot_logit_histograms(xt, xp, saveloc, title_):
    # plot the histogram
    xt = xt
    if len(xt.shape) == 1:
    ...
    gmm_xt = GaussianMixture(n_components=2, random_state=0).fit(xt)
    if xp is not None:
        gmm_xp = GaussianMixture(n_components=2, random_state=0).fit(xp)
        min_val_p = np.min(xp)
        max_val_p = np.max(xp)
    ...
    plt.plot(x_range_t, density_xt, alpha=1, color='#f77e00', linewidth=5, label='ID')
    ...
    if xp is not None:
        ...
        plt.plot(x_range_p, density_xp, alpha=1, color='#2e7dfe', linewidth=5, label='OOD')
