...
from tensorflow.keras.layers import Input, Conv2D, Dropout, Permute, DepthwiseConv2D, Reshape, Flatten, LSTM, BatchNormalization, Dense, Activation
...
class resnet_model:
    ...
    def __init__(self, type_='r', input_shape=(384, 384, 3), output_shape=1):
    ...
    def load_model_weighted_outs(self, saveloc, N_imgs=6):
        x = Input(shape=(N_imgs, 384, 384, 3))
    ...
    def make_model(self, N_imgs=6, to_ret=False): 
        inp = Input(shape=(N_imgs, 384, 384, 3))
        # resnet = tf.keras.applications.VGG16(weights='imagenet', pooling='avg', input_shape=(384, 384, 3), include_top=False)
        resnet = tf.keras.applications.ResNet50(weights='imagenet', pooling='avg', input_shape=(384, 384, 3), include_top=False)
        out = []
        ...
        attn_layer = Attention_linearCombination(64, 16, input_sizes = np.ones(6, dtype='int')*2048, return_weights = False,
    ...
    def train_model(self, data, label, m_saveloc, r_saveloc, num_epochs=25, val_data = None, val_label = None, weights=None):
        ...
        if self.type_ == 'r_weighted':
            ...
            train_dataset = tf.data.Dataset.from_generator(generator, output_types=({'input_1': tf.float32, 'labels': tf.float32, 'weights': tf.float32}, {'resnet-peopleCount': tf.float32}))
...
if __name__=='__main__':
    import sys
    # r_saveloc = './results/jhu_base_resnet50/'
    # m_saveloc = './models/jhu_base_resnet50/'
    # # X_train, y_train, X_val, y_val, X_test, y_test, data_loader = get_jhu_data()
    # X_train, y_train, X_val, y_val, X_test, y_test, data_loader = get_nwpu_data()
    
    r_saveloc = './results/jhu_tl_model/'
    m_saveloc = './models/jhu_tl_model/'
    source_loc = './models/nwpu_base_model/'

    obj = resnet_model()
    # obj.make_model()
    # sys.exit()
    obj.load_model(source_loc)

    X_train_id, y_train_id, X_val_id, y_val_id, X_test_id, X_train_ood, y_train_ood, X_val_ood, y_val_ood, X_test_ood, y_test_ood, rescale_obj = load_id_ood_people(log_scale=False)
    # obj.train_model(X_train_id, y_train_id, m_saveloc, r_saveloc, num_epochs=25, val_data=X_val_id, val_label=y_val_id)
    obj.train_model(X_train_ood, y_train_ood, m_saveloc, r_saveloc, num_epochs=25, val_data=X_val_ood, val_label=y_val_ood)


    # obj.load_model(m_saveloc)
    # y_train = rescale_obj.inv_scale(y_train_id) #np.exp(rescale_obj.inv_scale(y_train_id)) - 1.0
    # y_val = rescale_obj.inv_scale(y_val_id) #np.exp(rescale_obj.inv_scale(y_val_id)) - 1.0
    y_train = rescale_obj.inv_scale(y_train_ood)
    y_val = rescale_obj.inv_scale(y_val_ood)
    y_test = rescale_obj.inv_scale(y_test_ood)

    # pred_train_y = rescale_obj.inv_scale(obj.predict(X_train_id))   #np.exp(rescale_obj.inv_scale(obj.predict(X_train_id))) - 1.0
    # pred_val_y = rescale_obj.inv_scale(obj.predict(X_val_id))       #np.exp(rescale_obj.inv_scale(obj.predict(X_val_id))) - 1.0

    # y_val_global, pred_val_y_global = predict_global(y_val, pred_val_y, ids_val_id)

    # locs = np.where(np.squeeze(y_val) <= 10000)[0]
    # y_val = y_val[locs]
    # pred_val_y = pred_val_y[locs]
    ...
    print(y_test.shape, pred_test_y.shape)


    plot_predictions_paper(pred_train_y[:, 0], y_train[:, 0], r_saveloc+"train_r_target_paper25.png", title_='Train') #, addon=" (log scale)")
    plot_predictions_paper(pred_val_y[:, 0], y_val[:, 0], r_saveloc+"val_r_target_paper25.png", title_= 'Validation')
    # plot_predictions_paper(pred_val_y_global[:, 0], y_val_global[:, 0], r_saveloc+"val_r_target_paper_full.png", title_= 'Validation') #, addon=" (log scale)")
    plot_predictions_paper(pred_test_y[:, 0], y_test[:, 0], r_saveloc+"test_r_target_paper25.png", title_= 'Test')