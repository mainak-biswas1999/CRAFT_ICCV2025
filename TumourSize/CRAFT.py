...
class CRAFT(object):
    ...
    def __init__(self, output_shape=1, alpha=0.1, bs=24, lr=0.0001, n_bins=400, p_y_gmm=None):
...
def run_finetune(target_train_X, target_train_y, target_val_X, target_val_y, m_saveloc, r_saveloc, p_y_gmm, pretrain_loc, seed=0, perc_label=0.5, alpha=0.1):
    ...
    obj_cdist = CRAFT(p_y_gmm=p_y_gmm, alpha=alpha)
...
if __name__=='__main__':
    ...
    base_mloc = "./models/CRAFT_0.01/"
    base_rloc = "./results/CRAFT_0.01/"
    ...
    for i in [1, 2, 5, 10, 20]:
        ...
        for j in range(3):
            ...
            run_finetune(X_train_ood, y_train_ood, X_val_ood, y_val_ood, base_mloc+str(i)+"/"+str(j)+"/", base_rloc+str(i)+"/"+str(j)+"/", p_y_gmm, pretrain_loc='./models/cam16_base_patched/', seed=j, perc_label=(0.01*i), alpha=0.01)
