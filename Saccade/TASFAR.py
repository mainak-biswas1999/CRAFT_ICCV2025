...
def get_outputs(model, X, with_dropout=False, bs=128):
    all_ops = []
...
def run_train(target_train_X, target_train_y, target_test_X, target_test_y, target_val_X, target_val_y, p_y_gmm, m_saveloc, r_saveloc, model_Q=None, pretrain_loc=None, type_='r', alpha=1.0, target_dataset='DDir', model='EEGNET-LSTM', perc_data=1.0, perc_label=0.5, seed=0):
    ...
    weights = np.concatenate([np.ones((len(target_train_y_lab), 1)), 0.1*np.ones((len(pseudo_labels_tasfar), 1))], axis=0)
    ...
    enet_lstm_obj.train_model(target_train_X, target_train_y, m_saveloc, r_saveloc, num_epochs=n_epochs, val_data=target_val_X, val_label=target_val_y, weights=weights)
...
if __name__ == "__main__":
    ...
    import gc
    base_mloc = ["./Models/rebuttal/TASFAR_perc_sup_sacc/", "./Models/rebuttal/TASFAR_PyrCNN_perc_sup_sacc/", "./Models/rebuttal/TASFAR_perc_sup_wm/", "./Models/rebuttal/TASFAR_PyrCNN_perc_sup_wm/"]
    base_rloc = ["./Results/rebuttal/TASFAR_perc_sup_sacc/", "./Results/rebuttal/TASFAR_PyrCNN_perc_sup_sacc/", "./Results/rebuttal/TASFAR_perc_sup_wm/", "./Results/rebuttal/TASFAR_PyrCNN_perc_sup_wm/"]
    ...
    descriptions = ["EEGNET-LSTM: TASFAR", "PyrCNN: TASFAR", "EEGNET-LSTM: TASFAR", "PyrCNN: TASFAR"]
    ...
    for m in [0, 2]:
        ...
        for i in [1, 2, 5, 10]:
            ...
            for j in range(3):
                ...
                run_train(target_train_X, target_train_y, target_test_X, target_test_y, target_val_X, target_val_y, p_y_gmm, base_mloc[m]+str(i)+"/"+str(j)+"/", base_rloc[m]+str(i)+"/"+str(j)+"/", model_Q=base_mloc_tl[m]+str(i)+"/"+str(j)+"/", model=model_names[m], pretrain_loc=base_mloc_tl[m]+str(i)+"/"+str(j)+"/", target_dataset=dat_type[m], type_='r', alpha=0.1, perc_data=1.0, perc_label=(0.01*i), seed=j)
