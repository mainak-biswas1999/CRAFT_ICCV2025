...
if gpus:
    ...
    try:
        tf.config.experimental.set_virtual_device_configuration(gpus[0], [tf.config.experimental.VirtualDeviceConfiguration(memory_limit=12000)]) # Notice here
...
class progressive_mixup(object):
    ...
    def convex_combination(self, X1, y1, X2, y2, lam, N=None):
        ...
        lam = self.get_lambda(0, N=len(indices_1))
        lamx = np.expand_dims(lam, axis=np.arange(1,len(X1.shape)).tolist())
        lamy = np.expand_dims(lam, axis=np.arange(1,len(y1.shape)).tolist())
        X = lamx*X1[indices_1[0:N]] + (1.-lamx)*X2[indices_2[0:N]]
        y = lamy*y1[indices_1[0:N]] + (1.-lamy)*y2[indices_2[0:N]]
        return X, y
...
def run_train(target_train_X, target_train_y, target_test_X, target_test_y, target_val_X, target_val_y, p_y_gmm, m_saveloc, r_saveloc, pretrain_loc=None, type_='r', alpha=0.1, model='EEGNET-LSTM', target_dataset='DDir', perc_data=1.0, perc_label=0.5, seed=0):
    ...
    obj_cdist = progressive_mixup(type_=type_, model_name=model, alpha=alpha, target_dataset=target_dataset)
    ...
    plot_curves(loss_history['loss_conf'], "Confident Loss", "loss", n_epochs, r_saveloc+"/cons.png", x_label='Epoch')
    plot_curves(loss_history['loss_uncertain'], "Uncertain Loss", "loss", n_epochs, r_saveloc+"/uncertain.png", x_label='Epoch')
...
def run_test_paper(data_loader, target_train_X, target_train_y, target_test_X, target_test_y, target_val_X, target_val_y, p_y_gmm, m_saveloc, r_saveloc, type_='r', in_='pix', min_1=None, max_1=None, min_2=None, max_2=None, model='EEGNET-LSTM', target_dataset='DDir', target_plot_title=""):
    ...
    obj_cdist = progressive_mixup(type_=type_)
    obj_cdist.load_model(m_saveloc)
...
if __name__ == "__main__":
    ...
    base_mloc = ["./Models/rebuttal_final/mixup_perc_fixed_beta/"]
    base_rloc = ["./Results/rebuttal_final/mixup_perc_fixed_beta/"]
